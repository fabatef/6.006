...
class Timeline(object):
        ...
        def events_in_range(self, start, end):
                ...
                while start <= end: 
                        ...
                        event_node = self.events.find(start) # log n
                        ...
                        #update start
                        if self.events.successor(event_node) is None:
                                for events in event_node.value:
                                        result.append(Event(event_node.key, events[0]))
                                break
                        start = self.events.successor(event_node).key
                        if event_node.key <= end:
                                for events in event_node.value:
                                        result.append(Event(event_node.key, events[0]))
                return result
...
##test= [(59.99866009645795, 0), (41.106310954627276, 1), (121.11356441450394, 2), (8.120694891216381, 3), (28.271627566408696, 4), (63.18461482164673, 5), (61.9076268528119, 6), (61.43305810121645, 7), (76.66709829418413, 8), (14.421061725175266, 9), (88.2336685491408, 10), (42.145824981538794, 11)]
##bounds = [31.911621708713696, 33.78279417431307]
##my_timeline = Timeline()
##for (time, descript) in test:
##        my_timeline.add_event(Event(time, descript))
####my_timeline.add_event(Event(1, "second"))
####my_timeline.add_event(Event(1, "third"))
####my_timeline.add_event(Event(1, "fourth"))
##print ([(e.time, e.description) for e in my_timeline.events_in_range(31.911621708713696,33.78279417431307)])
